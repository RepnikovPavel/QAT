"""Train YOLOv5s + BitNet b1.58 (W1.58A8 body) on PASCAL VOC.
...
Usage (inside Docker qat-repro on the GPU server)::
    ...
    PYTHONPATH=/workspace/methods/bitnet_det:/workspace/methods/q2 \\
      python -u methods/bitnet_det/train_detect.py \\
        --epochs 50 --batch 16 --out /mnt/hdd2/qat_run/bitnet_voc

Smoke (few steps)::

    PYTHONPATH=/workspace/methods/bitnet_det:/workspace/methods/q2 \\
      python -u methods/bitnet_det/train_detect.py --limit 8 --epochs 1 --batch 4 \\
...
def load_pretrained_body(model: BitNetYOLOv5, ckpt_path: str, verbose: bool = True) -> dict:
    """Transfer COCO-pretrained yolov5s body weights where shapes match.

    BitConv2d / XNORConv2d store the kernel on ``.weight``; official Conv uses
    ``.conv.weight``.
    """
...
def main():
    ap = argparse.ArgumentParser(description="YOLOv5s + BitNet b1.58 on VOC")
    ...
    ap.add_argument(
        ...
        type=float,
        default=1e-3,
        help="Adam peak lr (BitNet FAQ uses higher LR than FP)",
    )
    ...
    ap.add_argument("--out", default="/mnt/hdd2/qat_run/bitnet_voc")
    ap.add_argument(
        "--pretrained",
    ...
    ap.add_argument("--fp32", action="store_true", help="FP32 baseline (no BitNet inject)")
    ap.add_argument(
        "--mode",
        default="bitnet",
        choices=["bitnet", "xnor"],
        help="bitnet=W1.58A8 BitConv2d; xnor=W1A1 XNORConv2d",
    )
    ap.add_argument("--no-stem-fp", action="store_true", help="also quantize stem Conv")
    args = ap.parse_args()
    ...
    print(
        f"Model: mode={args.mode} body={not args.fp32} n_bit_convs={n_bit} "
    ...
    with torch.no_grad():
        ...
        model.train()
        print(
            f"[smoke-fwd] outputs={len(out) if isinstance(out, (list, tuple)) else 1}",
            flush=True,
        )
