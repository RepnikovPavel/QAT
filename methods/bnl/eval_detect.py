"""mAP evaluation for YOLOv5s + BNL on PASCAL VOC.
...
def evaluate(
    ...
    Expects ``model`` in eval mode so Detect returns (decoded, raw).
    """
    ...
    with torch.no_grad():
        for imgs, targets in loader:
            imgs = imgs.to(device)
            out = model(imgs)
            # Detect eval: (decoded [N, nboxes, 5+nc], raw) or just decoded
    ...
    # yolov5 ap_per_class → (tp, fp, p, r, f1, ap, unique_classes)
    # ap shape: (nc, n_iou); with single IoU column n_iou=1 → mAP@0.5 only.
...
def build_model_from_ckpt(ckpt_path: str, device: torch.device) -> BNLYOLOv5:
...
def main():
    ap = argparse.ArgumentParser(description="Eval YOLOv5s+BNL mAP@0.5 on VOC")
    ...
    print(
        f"Loaded {args.ckpt} n_bnl={model.n_bnl} device={device}",
        flush=True,
    )

    # Warmup Detect grids at the target size
    model.eval()
    with torch.no_grad():
